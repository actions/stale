...
| `manual-stale-label`          | A label that you can apply manually on an issue/PR to mark it as stale instantly (cutting `days-before-stale`)                             | Optional |
| `manual-stale-issue-label`    | A label that you can apply manually on an issue to mark it as stale instantly (override `manual-stale-label`, cutting `days-before-stale`) | Optional |
| `manual-stale-pr-label`       | A label that you can apply manually on a PR to mark it as stale instantly (override `manual-stale-label`, cutting `days-before-stale`)     | Optional |
...
Define a custom label to shortcut the process of stale and mark immediately an issue as stale without waiting the days before stale:
    ...
    steps:
      - uses: actions/stale@v3
        with:
          ...
          manual-stale-issue-label: 'issue-stale-by-admin'
```
